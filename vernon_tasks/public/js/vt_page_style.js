/* vt_page_style.js — gray page background for selected desk pages.
   ...
   route change it paints the active page-container gray and makes the head and
   inner Frappe wrappers transparent so the gray shows through; cards keep their
   own background. Styles are applied inline (proven to take effect) rather than
   via a stylesheet class, so it does not depend on CSS load order/specificity. */

const VT_GRAY_PAGES = ["vt-home", "vt-projects", "vt-project-detail"];
...
function vt_apply_gray_bg() {
    ...
    if (!VT_GRAY_PAGES.includes(route[0])) {
        return;
    }
    ...
    const timer = setInterval(function () {
        ...
        if ($pc.length) {
            ...
            $pc.find(".page-head, .page-body, .layout-main-section")
