...
def calculate_points(doc, method) -> None:
    if doc.pdca_phase != "DONE" or not doc.completion_date:
        return
    ...
    _log(doc.name, doc.assigned_to, "earned", result["base"])
    if result["early_bonus"] > 0:
        _log(doc.name, doc.assigned_to, "early_bonus", result["early_bonus"])
    if result["late_penalty"] > 0:
        _log(doc.name, doc.assigned_to, "late_penalty", -result["late_penalty"])
    if result["revision_deduction"] > 0:
        _log(doc.name, doc.assigned_to, "revision_deduction", -result["revision_deduction"])
    ...
    add_points_to_period(
        user=doc.assigned_to, period=period,
    ...
    summary = get_or_create_today(doc.assigned_to, get_daily_target_hours(doc.assigned_to))
    frappe.db.set_value(
...
def apply_revision_deduction(task_name: str) -> None:
    ...
    _log(task_name, task.assigned_to, "revision_deduction", -deduction,
...
def override_points(task_name: str, new_points: float, reason: str, overridden_by: str) -> None:
    ...
    _log(task_name, task.assigned_to, "leader_override", delta,
    ...
    from vernon_tasks.workforce.doctype.user_point_summary.user_point_summary import add_points_to_period
    add_points_to_period(
        user=task.assigned_to, period=period,
...
def check_overdue_tasks() -> None:
    ...
    overdue = frappe.get_all(
        "VT Task",
        filters={"deadline": ["<", today()], "pdca_phase": ["not in", ["DONE"]], "assigned_to": ["!=", ""]},
        fields=["name", "title", "assigned_to", "project"],
    )
    for task in overdue:
        proj_leader = frappe.db.get_value("VT Project", task.project, "project_leader")
        recipients = [task.assigned_to]
