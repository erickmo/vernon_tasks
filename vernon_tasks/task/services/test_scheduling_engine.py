...
def make_task_and_project():
	setup()
	# Project is a root VT Item node; team_members is a child table on it.
	# Legacy VT Project.status → health_status.
	proj = frappe.get_doc({
	...
	# Task is a VT Item node parented to the project (ALLOWED_PARENTS["Task"]).
	# Legacy VT Task.assigned_to → owner_user; sprint/project Link → parent.
	task = frappe.get_doc({
...
class TestSchedulingEngine(FrappeTestCase):
	...
	def test_distribute_creates_schedule_entries(self):
		...
		task = frappe.get_doc("VT Item", self.task.name)
