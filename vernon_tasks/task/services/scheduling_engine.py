...
def check_capacity_conflict(user: str, day: date, additional_hours: float) -> bool:
    from vernon_tasks.workforce.doctype.work_profile.work_profile import get_daily_target_hours
    target = get_daily_target_hours(user)
    ...
    return (float(existing) + additional_hours) > target
...
def distribute_task_schedule(task_name: str) -> dict:
    ...
    for day in working_days:
        if check_capacity_conflict(task.assigned_to, day, hours_per_day):
            conflicts.append(str(day))
        task.append("schedule_entries", {
            "date": day, "allocated_hours": hours_per_day, "is_override": 0,
        })

    task.save(ignore_permissions=True)
    return {"conflicts": conflicts, "days_scheduled": len(working_days), "hours_per_day": hours_per_day}


def override_schedule_entry(task_name: str, day: date, new_hours: float) -> None:
    task = frappe.get_doc("VT Task", task_name)
    for row in task.schedule_entries:
        if getdate(row.date) == day:
            row.allocated_hours = new_hours
            row.is_override = 1
            break

    override_total = sum(r.allocated_hours for r in task.schedule_entries if r.is_override)
    remaining_hours = task.estimated_hours - override_total
    free_days = [r for r in task.schedule_entries if not r.is_override]

    if free_days and remaining_hours > 0:
        per_day = round(remaining_hours / len(free_days), 2)
        for row in free_days:
            row.allocated_hours = per_day
